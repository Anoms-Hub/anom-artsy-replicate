...
async function startServer() {
  ...
  // Stripe checkout routes (require auth middleware from context)
  app.use("/api/stripe/checkout", async (req, _res, next) => {
    // Attach user from JWT cookie for auth check in checkout handler
    try {
      const ctx = await createContext({ req, res: _res } as any);
